...
import { listLentEqByPageAPI } from '../service';
...
const SelectEquipmentModal: React.FC<IModalProps> = ({
    ...
    const [equipmentAllData, setEquipmentAllData] = useState<SelectedEquipmentItem[]>([]);
    const [selectEquipment, setSelectEquipment] = useState<SelectedEquipmentItem[]>([]);
    ...
    const getListLendEqData = async () => {
        try {
            setSpining(true);
            const { data } = await listLentEqByPageAPI({
                current: 1,
                deptId: deptId,
                pageSize: 99999
            });
    ...
    const onSearch = (value: string) => {
        ...
        const selectEquipmentStr = selectEquipment.map((item: any) => item.eqId).join();
        const tableData = equipmentAllData.filter((item: any) => 
            item.name.indexOf(value) != -1 &&
            selectEquipmentStr.indexOf(item.eqId.toString()) == -1
        );
            ...
            >
                ...
                <Spin size="large" spinning={spinning}>
                    <Table
                        ...
                        rowSelection={{
                            type: 'checkbox',
                            onChange: (selectedRowKeys: React.Key[], selectedRows: SelectedEquipmentItem[]) => {
